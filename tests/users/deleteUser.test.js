...
describe("DELETE /api/v1/users/:id", () => {
    ...
    beforeAll(async () => {
        const adminEmail = `delete-admin-${Date.now()}@devflow.test`;

        const developerEmail = `delete-developer-${Date.now()}@devflow.test`;
        ...
        await new Promise((resolve, reject) => {
            db.query("UPDATE users SET role = 'ADMIN' WHERE email = ?", [adminEmail], (err) => {
                if (err) {
                    reject(err);
                    return;
                }
        ...
        const developerResponse = await request(app).post("/api/v1/users").send({
