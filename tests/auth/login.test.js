...
const verifiedEmail = `login-${Date.now()}@devflow.test`;

const unverifiedEmail = `unverified-${Date.now()}@devflow.test`;
...
describe("POST /api/v1/users/login", () => {
    beforeAll(async () => {
        ...
        const unverifiedResponse = await request(app).post("/api/v1/users").send({
    ...
    test("logs in a verified user successfully", async () => {
        ...
        expect(
            response.headers["set-cookie"].some(
                (cookie) => cookie.startsWith("refreshToken=") && cookie.includes("HttpOnly")
            )
        ).toBe(true);

        expect(response.body.data).toBeDefined();

        expect(response.body.data.email).toBe(verifiedEmail);

        expect(response.body.data.name).toBe("Login Test User");
