...
describe("POST /api/v1/users", () => {
    ...
    test("creates a new user successfully", async () => {
        const email = `test-${Date.now()}@devflow.test`;
    ...
    test("rejects signup when name is missing", async () => {
        const email = `missing-name-${Date.now()}@devflow.test`;
    ...
    test("rejects signup when password is missing", async () => {
        const email = `missing-password-${Date.now()}@devflow.test`;
    ...
    test("rejects signup with an existing email", async () => {
        const email = `duplicate-${Date.now()}@devflow.test`;
